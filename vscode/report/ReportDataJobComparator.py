...
class ReportDataJobComparator extends ReportComparatorBase

\*\*
code> objects by the following skill, date, time, event name.
     * @param rd1
     * @param rd2
    ...
    protected int compareReportData(ReportData rd1, ReportData rd2)
        int result = compareStrings(rd1.getSkill(), rd2.getSkill())
        if(result == 0)
            result = compareDates(rd1.getDate(), rd2.getDate())
